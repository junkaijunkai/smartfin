---
name: start-feature
description: Start a new feature branch. Syncs the latest code from dev, then creates and switches to a new branch following the project naming convention. Use when beginning any new piece of work.
...
1. If the current branch is not `dev`, switch to `dev` first. If there are uncommitted changes on the current branch, stop and warn the user to commit or stash them before switching.
2. Run `git pull --rebase` to sync the latest code from remote `dev`. If there are conflicts, stop and report them — do not proceed.
